...
<script>
    ...
    <div class="container well">
       ...
       <div class="tab-content">
           ...
           $idPerguntas [$i] = $pergunta->id;
            ...
            <h2>{{$pergunta->questao}}</h2>
             <div class="container">
                ...
                <p class="text-left">
                 <input type="radio" id="resposta1{{$i}}" name="resposta{{$i}}" onclick="escolheOpcao('1{{$i}}','{{$i}}','{{$pergunta->opcao1}}')"/>
                 <label for="resposta1{{$i}}"  >{{$pergunta->opcao1}}</label>
                </div>

                <div id="opcao2{{$i}}" >
                    <p class="text-left">
                        <input type="radio" id="resposta2{{$i}}" name="resposta{{$i}}" onclick="escolheOpcao('2{{$i}}','{{$i}}','{{$pergunta->opcao2}}')" />
                        <label for="resposta2{{$i}}">{{$pergunta->opcao2}}</label>
                </div>

                <div id="opcao3{{$i}}">
                    <p class="text-left">
                        <input type="radio" id="resposta3{{$i}}" name="resposta{{$i}}" onclick="escolheOpcao('3{{$i}}','{{$i}}','{{$pergunta->opcao3}}')" />
                        <label for="resposta3{{$i}}">{{$pergunta->opcao3}}</label>
                </div>

                <div id="opcao4{{$i}}">
                    <p class="text-left">
                        <input type="radio" id="resposta4{{$i}}" name="resposta{{$i}}" onclick="escolheOpcao('4{{$i}}','{{$i}}','{{$pergunta->opcao4}}')" />
                        <label for="resposta4{{$i}}">{{$pergunta->opcao4}}</label>
                </div>

                <div id="opcao5{{$i}}">
                    <p class="text-left">
                        <input type="radio" id="resposta5{{$i}}" name="resposta{{$i}}" onclick="escolheOpcao('5{{$i}}','{{$i}}','{{$pergunta->opcao5}}')" />
        ...
        {!!Form::open(array('url' => 'exame','onsubmit'=>'return valido()')) !!}
         ...
         {!!Form::submit('Entregar',['class'=>'btn btn-primary', 'id'=>'entregar']) !!}
        </div>
        {!!Form::close()!!}
    </div>
<div class="modal fade" id="mensagem" role="dialog">
    ...
    <script>
        ...
        window.onload = function () {
            var Minutes = 60* 1, display = document.querySelector('#timer');
